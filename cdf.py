...
def _extract_cat_dfs():
    ...
    for cat in cat_store.get_usable_cats():
        ...
        base_props = {prop for prop, freq in cat_stats['property_frequencies'].items() if freq >= CDF_ALPHA}
        base_types = {(rdf_util.PREDICATE_TYPE, t) for t, freq in cat_stats['type_frequencies'].items() if freq >= CDF_ALPHA}
...
def _get_overall_features_count(feats, cat=None):
    valid_res_idxs = set()
    if cat:
...
def _get_feature_set_as_string(feature_set):
...
def _extract_axioms_with_rules(cat_dfs):
    ...
    for word_patterns in cdf_rule_candidates:
        ...
        valid_axiom_patterns = [pattern for pattern, support in cdf_rule_candidates[word_patterns].items() if support >= CDF_MIN_SUPPORT and (support / total_support) >= CDF_BETA]
...
def _apply_rules(pattern_dict, cat):
